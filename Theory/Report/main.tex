...
\end{equation}
\end{defn}
For two corrolated spikes, we define:
...
\paragraph{Reduction to a spiked Wigner model (splitting the observation into noiseless $+$ noise):}
...
\end{itemize}
The strategy is: solve $\rdmmat P$ exactly (Sections up to \eqref{eq:overlaps-signals}), then feed that solution into the BBP theory to get $\rdmmat Y$ (from \eqref{eq:bbp} on). To keep the two apart, we reserve a distinct set of symbols for each:
\begin{center}
\begin{tabular}{l|l|l}
...
eigenvectors    & $\hat{\vect v}_\pm$ (exact, \eqref{eq:v-final})  & $\hat{\vect u}_\pm$ (asymptotic, \eqref{eq:bbp-vec}) \\
top eigenvector & ---                                              & $\hat{\vect x} := \hat{\vect u}_+$, the estimator of the signals \\
...
\paragraph{Factorised form (noiseless $\rdmmat P$):}
...
\paragraph{Empirical vs theoretical overlap (signals only, no matrix involved):}
...
\paragraph{Going to the plane of the signals (méthode de Celia) --- still noiseless $\rdmmat P$:} Let $\vect v$ be a unit eigenvector of $\rdmmat P$ with eigenvalue $\theta \neq 0$. Since
\begin{equation}
...
\paragraph{Final expression for the eigenvectors of the noiseless $\rdmmat P$:} Combining \eqref{eq:ratio} with the normalisation, the unit-norm eigenvectors of $\rdmmat P$ associated with $\theta_\pm$ are, up to an overall sign,
\begin{equation}
    ...
    \label{eq:v-final}
\end{equation}
In plain words: $\rdmmat P$ only acts inside the plane spanned by $\vect{x_1}$ and $\vect{x_2}$, so each of its two eigenvectors is simply a \emph{mixture} of the two signals. All the content of \eqref{eq:v-final} is the single number $r_\pm = c_1^\pm/c_2^\pm$, which fixes the weight of the mixture; the square root is only there to impose $\norm{\hat{\vect v}_\pm} = 1$, and it carries the extra term $2\rho\, r_\pm$ precisely because $\vect{x_1}$ and $\vect{x_2}$ are \emph{not} orthogonal. Since $\theta_+ \ge \max(a,b)$ and $\theta_- \le \min(a,b)$, we have $r_+ \ge 0$ and $r_- \le 0$: $\hat{\vect v}_+$ is the constructive combination of the two signals, and $\hat{\vect v}_-$ the destructive one. We insist that $\hat{\vect v}_\pm$ are eigenvectors of the \emph{noiseless} $\rdmmat P$; they are not observable, and they are not yet the estimator of the signals.

\paragraph{The decoupled case $\rho = 0$ (noiseless $\rdmmat P$):} The ratio $r_\pm = \rho a/(\theta_\pm - a)$ becomes the indeterminate form $0/0$ when $\rho = 0$ and $\theta_\pm = a$, so at $\rho = 0$ it has to be read as a limit. It is cleaner to go back to \eqref{eq:eig-system}, which for $\rho = 0$ decouples into $a c_1 = \theta c_1$ and $b c_2 = \theta c_2$: then $\{\theta_+,\theta_-\} = \{a,b\}$ and the eigenvectors are $\vect{x_1}$ and $\vect{x_2}$ themselves, i.e.\ the two tasks separate completely. Numerically, the safe way to obtain $(c_1^\pm, c_2^\pm)$ for \emph{any} $\rho$ is to diagonalise the $2\times2$ matrix $\rdmmat K \rdmmat G$ directly instead of evaluating $r_\pm$.
...
\end{equation}
The point of \eqref{eq:overlaps-signals} is that the \emph{coefficients} $c_i^\pm$ (how much of each signal we put into the mixture) are not the same thing as the \emph{overlaps} $\langle \hat{\vect v}_\pm, \vect{x_i}\rangle$ (how much of each signal we actually see), again because $\vect{x_1}$ and $\vect{x_2}$ are not orthogonal; the factors $\theta_\pm/a$ and $\theta_\pm/b$ convert one into the other. Equation \eqref{eq:overlaps-signals} is the last exact, noiseless result of this section: everything that follows concerns the observed matrix $\rdmmat Y$ and holds only asymptotically.

\medskip
\noindent
Equation \eqref{eq:overlaps-signals} also gives a useful identity. Multiplying its first relation by $a\,\langle \hat{\vect v}_\pm, \vect{x_1}\rangle$, its second by $b\,\langle \hat{\vect v}_\pm, \vect{x_2}\rangle$, adding, and using the normalisation $c_1^2 + 2\rho c_1 c_2 + c_2^2 = 1$, we get
\[
...
\]
In other words, $\theta_\pm$ is exactly the signal-to-noise ratio of each task weighted by how well the direction $\hat{\vect v}_\pm$ captures that task. This is what makes $\theta_\pm$ an \emph{effective SNR}, and it is the bridge between the noiseless geometry \eqref{eq:overlaps-signals} and the noisy spectrum \eqref{eq:bbp} below: $\theta_\pm$ is computed entirely from $\rdmmat P$, but it is the only feature of $\rdmmat P$ that the spectrum of $\rdmmat Y$ remembers.

\medskip
\noindent
Still on the noiseless side: since $\rdmmat P$ is symmetric and $\theta_+ \neq \theta_-$, the two eigenvectors $\hat{\vect v}_\pm$ are orthogonal. The remaining $n-2$ eigenvectors span $\operatorname{span}(\vect{x_1},\vect{x_2})^\perp$ and have eigenvalue $0$. For a finite $n$, all the expressions above hold exactly with $\rho$ replaced by the empirical overlap $\hat\rho$; by \eqref{eq:overlap}, the expressions written with $\rho$ are their $n \to \infty$ limits.

\paragraph{Switching on the noise: the outliers of the noisy matrix $\rdmmat Y$.}
We now leave the noiseless matrix and turn to the observed one. Because $\rdmmat W$ is a Wigner matrix (rotationally invariant Gaussian noise), the outlier eigenvalues of $\rdmmat Y = \rdmmat W + \rdmmat P$ depend on the finite-rank perturbation $\rdmmat P$ only through its non-zero eigenvalues $\theta_\pm$ \cite{benaychgeorges2011eigenvalues, capitaine2009largest}. The Baik--Ben Arous--P\'ech\'e (BBP) transition \cite{baik2005phase, feral2007largest} then applies to each $\theta_\pm$ separately: with the normalisation of Definition~\ref{def:spiked-wigner}, the semicircle bulk has edge $2$ and
\begin{equation}
    ...
    \label{eq:bbp}
\end{equation}
The practical reading of \eqref{eq:bbp} is that, as far as the spectrum of $\rdmmat Y$ is concerned, the two correlated tasks behave like two \emph{independent} rank-one spiked Wigner models with SNRs $\theta_+$ and $\theta_-$: Theorem~\ref{thm:bbp} applies verbatim with $\lambda$ replaced by $\theta_\pm$. The correlation therefore acts in both directions: it can push $\theta_+$ above the detection threshold even when $a$ and $b$ are individually sub-critical, while pushing $\theta_-$ deeper into the bulk. In the $(a,b)$ plane the detection boundary $\theta_+ = 1$ is obtained by setting $\theta = 1$ in the characteristic polynomial of $\rdmmat K\rdmmat G$, which gives the curve
\begin{equation}
    1 - (a+b) + ab\,(1-\rho^2) = 0 ,
    \qquad \text{with } a + b \le 2,
    \label{eq:boundary}
\end{equation}
the condition $a + b \le 2$ selecting the branch on which the root equal to $1$ is the \emph{larger} one (the two roots sum to $a+b$, so the other root is $a+b-1$). The branch $a+b \ge 2$ of the same curve is $\theta_- = 1$, where the second outlier appears. For $\rho = 0$ the curve \eqref{eq:boundary} factorises as $(1-a)(1-b) = 0$ and reduces to the two BBP half-lines $a = 1,\, b \le 1$ and $b = 1,\, a \le 1$; for $\rho = 1$ it reduces to the line $a + b = 1$.

\paragraph{The eigenvectors of the noisy matrix $\rdmmat Y$.}
The same theory \cite{benaychgeorges2011eigenvalues} describes the eigenvectors of the noisy matrix. Let $\hat{\vect u}_\pm$ denote the unit eigenvector of $\rdmmat Y$ associated with $\lambda_\pm^{\mathrm{out}}$; it is the noisy counterpart of the noiseless $\hat{\vect v}_\pm$, and \eqref{eq:bbp-vec} below says how close the two are. When $\theta_\pm > 1$ (and $\theta_+ \neq \theta_-$),
\begin{equation}
    ...
    \label{eq:bbp-vec}
\end{equation}
while for $\theta_\pm \le 1$ the overlap with $\hat{\vect v}_\pm$ vanishes; this is again Theorem~\ref{thm:bbp} with $\lambda$ replaced by $\theta_\pm$.

\medskip
\noindent
Combining the noiseless geometry with the noisy damping is now straightforward. The noiseless eigenvectors $\hat{\vect v}_+$ and $\hat{\vect v}_-$ form an orthonormal basis of $\operatorname{span}(\vect{x_1},\vect{x_2})$, so each signal decomposes as
\[
    \vect{x_i} = \langle \vect{x_i}, \hat{\vect v}_+\rangle \hat{\vect v}_+ + \langle \vect{x_i}, \hat{\vect v}_-\rangle \hat{\vect v}_- .
\]
Taking the scalar product with the top eigenvector $\hat{\vect x} := \hat{\vect u}_+$ of the \emph{noisy} matrix $\rdmmat Y$,
\[
    \langle \hat{\vect x}, \vect{x_i}\rangle
    = \underbrace{\langle \hat{\vect u}_+, \hat{\vect v}_+\rangle}_{\to\ \sqrt{1-\theta_+^{-2}}\ \text{by }\eqref{eq:bbp-vec}}
      \langle \hat{\vect v}_+, \vect{x_i}\rangle
    + \underbrace{\langle \hat{\vect u}_+, \hat{\vect v}_-\rangle}_{\to\ 0\ \text{by }\eqref{eq:bbp-vec}}
      \langle \hat{\vect v}_-, \vect{x_i}\rangle ,
\]
where the component of $\hat{\vect x}$ orthogonal to the plane contributes nothing because $\vect{x_i}$ lies in the plane. With \eqref{eq:overlaps-signals} this gives
\begin{equation}
    ...
    \label{eq:m12}
\end{equation}
for $\theta_+ > 1$, and $m_1, m_2 \to 0$ for $\theta_+ \le 1$. The structure of \eqref{eq:m12} is worth emphasising: it is a product of two independent effects. The factor $\sqrt{1-\theta_+^{-2}}$ is the \emph{noise damping} --- how much of the direction $\hat{\vect v}_+$ survives the Wigner noise, and it is the same for both tasks. The factors $(\theta_+/a)|c_1^+|$ and $(\theta_+/b)|c_2^+|$ are the \emph{pure geometry} of the noiseless signal matrix, and they say how the single recovered direction is shared between $\vect{x_1}$ and $\vect{x_2}$, through the ratio $r_+ = c_1^+/c_2^+$.

\medskip
\noindent
Detection by the top eigenvector is therefore possible if and only if $\theta_+ > 1$, i.e.\ above the curve \eqref{eq:boundary}. One caveat: above that curve $m_2 > 0$ always (since $c_2^+ \neq 0$), whereas $m_1 > 0$ requires $r_+ \neq 0$, i.e.\ $\rho > 0$. In the decoupled case $\rho = 0$ the top eigenvector aligns with the stronger signal only ($\vect{x_1}$ if $a > b$, $\vect{x_2}$ if $a < b$) and carries no information at all about the other one; recovering the weaker signal then requires the \emph{second} eigenvector, i.e.\ $\theta_- > 1$.

\paragraph{Numerical check.} Equations \eqref{eq:v-final}--\eqref{eq:m12} are checked against simulations in \texttt{Theory/check\_bulk.py}, which uses exactly the same conventions ($a = \alpha\lambda_1$, $b = \sqrt{1-\alpha^2}\lambda_2$, unit-norm signals, semicircle edge at $2$) and obtains $(c_1^+,c_2^+)$ by diagonalising $\rdmmat K\rdmmat G$ rather than through $r_+$. Its simulations always build the \emph{noisy} $\rdmmat Y$ (\texttt{naive\_matrix} $= \alpha \rdmmat Y_1 + \sqrt{1-\alpha^2}\rdmmat Y_2$) and compare it to the theory curves; the noiseless $\hat{\vect v}_\pm$ enter only as the reference directions of \texttt{overlaps\_vs\_lambda.png}.
